...
function copyIndexTo404Plugin(): Plugin {
  return {
    ...
    writeBundle: async (options) => {
      ...
      const indexHtml = nodePath.resolve(distDir, 'index.html');
      const errorHtml = nodePath.resolve(distDir, '404.html');

      try {
        await fs.access(indexHtml);
        await fs.copyFile(indexHtml, errorHtml);
        console.log('[copy-index-to-404] Copied index.html to 404.html for SPA routing');
      } catch (e) {
        console.warn('[copy-index-to-404] Could not copy index.html to 404.html', e);
      }
    }
  };
}
